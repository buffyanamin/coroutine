...
struct end_awaitable_t final {
  ...
  public:
    ...
    coro::coroutine_handle<void> await_suspend([[maybe_unused]] coro::coroutine_handle<void>) noexcept;
...
class paused_action_t final {
  public:
    class promise_type final {
        coro::coroutine_handle<void> next = nullptr; // task that should be continued after `final_suspend`
      ...
      public:
        ...
        void set_next(coro::coroutine_handle<void> task) noexcept;
    };

    struct chained_awaitable_t final {
        coro::coroutine_handle<promise_type> action; // handle of the `paused_action_t`

      public:
        constexpr bool await_ready() const noexcept {
            return false;
        }
        coro::coroutine_handle<void> await_suspend(coro::coroutine_handle<promise_type> coro) noexcept;
  ...
  public:
    ...
    coro::coroutine_handle<void> handle() const noexcept;

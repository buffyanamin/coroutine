...
/**
 * @brief   `suspend_never`(initial) + `std::experimental::suspend_always`(final)
 * @ingroup Return
 */
class promise_na {
  public:
...
/**
 * @brief   `std::experimental::suspend_always`(initial) + `suspend_never`(final)
 * @ingroup Return
 */
class promise_an {
  public:
...
/**
 * @brief   `std::experimental::suspend_always`(initial) + `std::experimental::suspend_always`(final)
 * @ingroup Return
 */
class promise_aa {
  public:
...
/**
 * @brief   A type to acquire `coro::coroutine_handle<void>` from anonymous coroutine's return.
 *          Requires manual `destroy` of the coroutine handle.
 *
 * @ingroup Return
 * @see coro::coroutine_handle<void>
 * @see promise_na
 */
class frame_t : public coro::coroutine_handle<void> {
  public:
    /**
     * @brief Acquire `coro::coroutine_handle<void>` from current object and expose it through `get_return_object`
     */
    class promise_type : public promise_na {
      public:
        ...
        /**
         * @brief Acquire `coro::coroutine_handle<void>` from current promise and return it
         * @return frame_t
         */
        frame_t get_return_object() noexcept {
            return frame_t{coro::coroutine_handle<promise_type>::from_promise(*this)};
        }
    };

  public:
    explicit frame_t(coro::coroutine_handle<void> frame = nullptr) noexcept
        : coro::coroutine_handle<void>{frame} {
    }
};

/**
 * @brief Suspend after invoke and expose its `coro::coroutine_handle<void>` through return
 * @ingroup Return
 * @see coro::coroutine_handle<void>
 * @see frame_t
 * @see promise_aa
 */
class passive_frame_t : public coro::coroutine_handle<void> {
  public:
    ...
    explicit passive_frame_t(coro::coroutine_handle<void> frame = nullptr) noexcept
        : coro::coroutine_handle<void>{frame} {
    }
};
...
concept awaitable = requires(T a, coro::coroutine_handle<void> h) {
    { a.await_ready() } ->bool;
